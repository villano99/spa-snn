...
from dailyaction_dataset import DailyDVSHDF5Dataset
...
def make_dataloaders(args, for_stdp=False):
    """Crea DataLoaders (HDF5 streaming)."""
    return _make_dataloaders_hdf5(args)


def _make_dataloaders_hdf5(args):
    """DataLoaders usando DailyDVSHDF5Dataset (HDF5 preprocesado)."""
...
class GraphSNN(nn.Module):
    def __init__(self, N_in, M1, M2, M3, mask1, mask2, mask3,
        ...
        # Nota: Las mascaras pueden estar en GPU, mover a CPU para init
...
# ========================
# Pre-entrenamiento R-STDP (Mejorado)
# ========================
...
def spa_supervised_train(model, train_loader, val_loader, device, args):
    ...
    for ep in range(1, args.epochs_sup + 1):
        ...
        for batch in pbar:
            ...
            # Extraer spk1 de L1 pre-entrenada con STDP (congelada)
            with torch.no_grad():
                out, spk1, _, _, _ = model(x, return_internals=True)
            
            # Entrenar red profunda E-Prop sobre spk1
...
def main():
    ...
    if args.turbo:
        ...
        print("[CONFIG] Configuracion predeterminada activa (Batch 64).")
    ...
    # Optimizacion de compilacion de PyTorch
    # Nota: torch.compile puede causar errores de Triton en Windows.
    # Desactivado para mayor estabilidad.
