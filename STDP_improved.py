...
def visualize_receptive_fields(W, mask, W_model, H_model, P, save_dir="./rf_viz", n_neurons=16, auto_crop=False, event_density=None):
    ...
    for i, idx in enumerate(indices):
        ...
        # Gamma for visibility (brighten faint signals)
...
def stdp_pretrain_improved(model, loader, device, args, W, H):
    ...
    print(f"[STDP] L1: η={args.eta_stdp}, target={args.stdp_target_rate}")
    print(f"[STDP] L2: η={args.eta_stdp}, target={args.stdp_target_rate*0.5} (Traces Lentas)")
